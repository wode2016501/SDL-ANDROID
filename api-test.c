#include <stdio.h>
#define dlog 1
 //Android_JNI_GetManifestEnvironmentVariables  on off
#define dlog1 0
...
int  Android_JNI_GetManifestEnvironmentVariables(){
	if(dlog1)fprintf(stderr," Android_JNI_GetManifestEnvironmentVariables\n");
	return 0;
}
...
int Android_JNI_GetDisplayOrientation(){
	if(dlog)fprintf(stderr," Android_JNI_GetDisplayOrientation\n");
	return 0;
}
int Android_JNI_GetAccelerometerValues(){
	if(dlog)fprintf(stderr," Android_JNI_GetAccelerometerValues\n");
	return 0;
}
int Android_JNI_RequestPermission(){
	if(dlog)fprintf(stderr," Android_JNI_RequestPermission\n");
	return 0;
}
int Android_DetectDevices(){
	if(dlog)fprintf(stderr," Android_DetectDevices\n");
	return 0;
}
int Android_JNI_CaptureAudioBuffer(){
	if(dlog)fprintf(stderr," Android_JNI_CaptureAudioBuffer\n");
	return 0;
}
int Android_JNI_FlushCapturedAudio(){
	if(dlog)fprintf(stderr," Android_JNI_FlushCapturedAudio\n");
	return 0;
}
int SDL_AndroidGetInternalStoragePath(){
	if(dlog)fprintf(stderr," SDL_AndroidGetInternalStoragePath\n");
	return 0;
}
int Android_JNI_AudioSetThreadPriority(){
	if(dlog)fprintf(stderr," Android_JNI_AudioSetThreadPriority\n");
	return 0;
}
